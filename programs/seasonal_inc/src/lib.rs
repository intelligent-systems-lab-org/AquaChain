...
declare_id!("ArBfA7Qd6t33oKiWsdFc7RxTzECzfEC3BHgmmKvZxqNQ");
...
pub mod seasonal_inc {
    ...
    pub fn use_water(
        ...
        amount: u64,
        reservoir_init_capacity: u64,       // Added for initial Reservoir capacity
        reservoir_capacity: u64,            // Added for current Reservoir capacity
    ) -> Result<()> {
        let consumer = &mut ctx.accounts.consumer;
        let tariff = &ctx.accounts.tariff;

        // Validation: Ensure that initial reservoir capacity is not 0 and current reservoir capacity is never larger than initial
        require!(reservoir_init_capacity > 0, CustomError::InvalidCapacity);
        require!(reservoir_init_capacity >= reservoir_capacity, CustomError::InvalidCapacity);

        consumer.reservoir_capacity = reservoir_capacity;   // Added for reservoir capacity
        consumer.reservoir_init_capacity = reservoir_init_capacity;     // Added for initial reservoir capacity
        ...
        let consumer_wtk_balance = ctx.accounts.consumer_wtk.amount;    // Added for Watertoken balance
        let rate = if consumer_watc_balance >= amount {
            tariff.water_rate
        } else {
            consumer_wtk_balance*(1 - (consumer.reservoir_capacity/consumer.reservoir_init_capacity))     // Changed alternate rate to WaterkToken*(1 - ReservoirCap%)
        };
...
pub struct Consumer {
    pub block_rate: u64,
    pub reservoir_init_capacity: u64,
